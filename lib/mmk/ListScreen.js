...
import { isRtlLang } from '../i18n/lang'
import { RowEntry } from './ListScreenParts/RowEntry'
import {
  BASE_FONT_SIZE,
  BLACK,
  IS_ROUND, SCREEN_HEIGHT, SCREEN_MARGIN_X,
  SCREEN_MARGIN_Y, SCREEN_WIDTH
} from './UiParams'

export class ListScreen {
  constructor (rtl = undefined, yOffset = 0) {
    if (!rtl) rtl = isRtlLang()
  ...
  headline (text, description = undefined) {
    this.row({
      text,
      description,
