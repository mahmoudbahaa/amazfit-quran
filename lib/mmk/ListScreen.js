...
export class ListScreen {
  ...
  finalize(listIndex = undefined, posY = undefined, scrollUpdateDuration = undefined) {
    ...
    if (listIndex === undefined) {
      ...
      if (lastRow.setVisible) {
        lastRow.setVisible(true);
      }
  ...
  row(userConfig) {
    if (userConfig.alignH === undefined) {
      userConfig.alignH = this.rtl ? hmUI.align.RIGHT : hmUI.align.LEFT;
    }
  ...
  _onScroll(scrollTop) {
    const entriesLength = (this.realEntriesNum === undefined) ? (this.entries.length - 1) : this.realEntriesNum;
    ...
    this.entries.slice(0, this.realEntriesNum + 1).forEach((entry, i) => {
      ...
      if (entry.setVisible && i < entriesLength && !hidden) {
