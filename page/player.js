...
import {
  createWidget, event, prop, setStatusBarVisible, widget,
...
import {
  DECREASE_VOLUME, EXIT, INCREASE_VOLUME, MIN_TIMEOUT_DURATION, NEXT, NUM_VERSES, PAUSE, PLAY, PREVIOUS, START, STOP,
} from '../lib/constants';
...
Page({
  ...
  onInit(paramsString) {
    ...
    this.state.number = parseInt(params.number, 10);
  ...
  getServiceParam(action) {
    ...
    if (action === START) {
      ...
      if (this.state.verse !== undefined) {
        extraParams += `&verse=${this.state.verse}`;
      }
    }
  ...
  getVerseLabel(v) {
    const label = `${_(v.split(':')[1])}${_('/')}${_(NUM_VERSES[parseInt(v.split(':')[0], 10) - 1])}`;
    if (!isRtlLang()) {
      return label;
    }
    ...
    const updater = that => {
      ...
      if (verse === undefined) {
        ...
        if (elapsed > lastVerseInfo.mapping[lastVersePartIndex]) {
          ...
          lastVersePartIndex++;
          lastVerseText = lastVerseInfo.texts[lastVersePartIndex];
          verseTextText = lastVerseText;
        } else {
