...
export class EasyFlashStorage { // @add 1.4.0
  ...
  dataSize (key, unit = 'B') {
    if (this.hasKey(key)) {
      const stat = statSync({ path: this.#directory + '/' + this.#keys.indexOf(key) })
...
export class EasyTSDB { // @add 1.6.0
  ...
  #collectDataPointsForRange (start_time, end_time) {
    const bugfixed_start_time = new Date(start_time)
...
String.prototype.fromHex = function () {
  const hex_string = this.toString() // ensure we have a string
  let result = ''
 ...
 * - @rem index dropped, the complexity and memory requirement not worth it - full ssd approach from now. need it for a test? find easy-storage-loadingWidget.js
 * - Storage:	--------------------------------------------------------------
