...
import { openSync, openAssetsSync, rmSync, mkdirSync, closeSync, O_RDONLY, O_WRONLY, O_CREAT, writeSync } from '@zos/fs';
...
export class Path {
  ...
  open(flags) {
    if (this.scope === "data") {
      this._f = openSync(this.relativePath, flags);
    } else {
      this._f = openAssetsSync(this.relativePath, flags);
    }
  ...
  remove() {
    ...
    try {
      rmSync(isMiBand7 ? this.absolutePath : this.relativePath);
      return true;
    } catch (e) {
      return false;
    }
  ...
  mkdir() {
    return mkdirSync(isMiBand7 ? this.absolutePath : this.relativePath);
  }
